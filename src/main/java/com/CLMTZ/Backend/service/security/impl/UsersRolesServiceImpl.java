...
public class UsersRolesServiceImpl implements IUsersRolesService {
    ...
    @Override
    public List<UsersRolesDTO> findAll() { return repository.findAll().stream().map(this::toDTO).collect(Collectors.toList()); }

    @Override
    public UsersRolesDTO findById(Integer id) { return repository.findById(id).map(this::toDTO).orElseThrow(() -> new RuntimeException("UsersRoles not found with id: " + id)); }

    @Override
    public UsersRolesDTO save(UsersRolesDTO dto) {
    ...
    @Override
    public UsersRolesDTO update(Integer id, UsersRolesDTO dto) {
    ...
    private UsersRolesDTO toDTO(UsersRoles e) {
        UsersRolesDTO d = new UsersRolesDTO();
