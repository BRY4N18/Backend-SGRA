...
public class ReinforcementRequestController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ReinforcementRequestDTO> findById(@PathVariable Integer id) { return ResponseEntity.ok(service.findById(id)); }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ReinforcementRequestDTO> update(@PathVariable Integer id, @RequestBody ReinforcementRequestDTO dto) { return ResponseEntity.ok(service.update(id, dto)); }
